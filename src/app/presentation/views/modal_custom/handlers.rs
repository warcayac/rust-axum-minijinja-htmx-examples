...
pub async fn root_hdlr(State(state): TAppStateState) -> impl IntoResponse {
    let template = state.env.get_template("pages/modal_custom/index.html");
    let context = context! {
        target => APP_ROUTES["modalcustom"].full_pathname(0),
    };
...
pub async fn modal_hdlr(State(state): TAppStateState) -> impl IntoResponse {
    let template = state.env.get_template("pages/modal_custom/partials/modal.html");
    let context = context! {
    };
